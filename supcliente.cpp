...
SupCliente::~SupCliente()
{
  ...
  if (isConnected())
  {
    ...
    /* ACRESCENTAR */
    sock_client.write_int16(CMD_LOGOUT);
                          ...
                          const std::string& Senha)
{
  ...
  try
  {
    ...
    /* ACRESCENTAR */
    iResult = sock_client.connect(IP,SUP_PORT);
    ...
    /* ACRESCENTAR */
    iResult = sock_client.write_int16(CMD_LOGIN);
    ...
    /* ACRESCENTAR */
    iResult = sock_client.read_uint16(cmd);
    ...
    /* ACRESCENTAR */
    thr_client = std::thread([this] { main_thread(); });
    if (!thr_client.joinable()) throw 108;
  }
  catch (int err)
  {
    // Encerra o cliente
...
void SupCliente::setValvOpen(bool isV1, bool Open)
{
  ...
  try
  {
    ...
    /* ACRESCENTAR */
    iResult = sock_client.write_int16(cmd);
    ...
    /* ACRESCENTAR */
    iResult = sock_client.write_uint16(Open ? 1 : 0);
...
void SupCliente::setPumpInput(uint16_t Input)
{
  ...
  try
  {
    ...
    /* ACRESCENTAR */
    iResult = sock_client.write_int16(CMD_SET_PUMP);
...
void SupCliente::main_thread(void)
{
  ...
  while (!encerrarCliente && isConnected())
  {
    ...
    try
    {
      ...
      /* ACRESCENTAR */
      iResult = sock_client.write_int16(CMD_GET_DATA);
      ...
      /* ACRESCENTAR */
      iResult = sock_client.read_uint16(cmd,1000*SUP_TIMEOUT);
